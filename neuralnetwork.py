...
def h(theta1,theta2,x):#forward propogation
...
def flatten_x(x): #one large 1d array to send it to fmin_cg
    return x.flatten().reshape(input_units*training_examples,1)
   
def reshape_x(x):#get back tthe original matrix form
    return x.reshape(training_examples,input_units)
    
def flatten_thetas(theta1,theta2): #one big 1d array for thetas again for ur minimization function
	theta1=theta1.flatten().reshape((input_units+1)*hidden_units,1)
	theta2=theta2.flatten().reshape((hidden_units+1)*output_units,1)
	thetas=np.zeros((theta1.size+theta2.size,1))
	thetas[:theta1.size]=theta1
	thetas[theta1.size:]=theta2
	return thetas

def reshape_thetas(thetas):#getting back theta1 and theta2
	theta1=thetas[:(input_units+1)*hidden_units].reshape(hidden_units,input_units+1)
	theta2=thetas[(input_units+1)*hidden_units:].reshape(output_units,hidden_units+1)
	return theta1,theta2
	
def gen_random_thetas():  #generate random theta values
	epsilon = 0.1
...
def predict(x,y,rval):
	...
	m=x[:,0].size
	print "no of testing examples=",m
	count=0
	for i in range(0,m):
		actual=y[i][0]
...
def preprocess_data(x,y):
	...
	y_train = np.zeros((training_examples,1))
	x_test = np.zeros((testing_examples,400))
	y_test = np.zeros((testing_examples,1))
...
save(rval)#save the backup of trained valu
